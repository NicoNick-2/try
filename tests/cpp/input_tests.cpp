...
TEST(read_ints, ThrowIfNotEnoughLines) {
    ...
    try {
        ...
        std::string msg = e.what();
        EXPECT_TRUE(msg.find("Expected 3 lines but got 2") !=
                    std::string::npos);
    }
...
TEST(read_int_pairs, ReadCorrectly) {
    std::string file = create_file("1 2\n"
                                   "3 4\n"
                                   "5 6\n");
...
TEST(read_int_pairs, ThrowIfInvalidIntegerPair) {
    std::string file = create_file("10 11\n"
                                   "abc\n"
                                   "30 21\n");
    ...
    try {
        ...
        std::string msg = e.what();
        EXPECT_TRUE(msg.find("Invalid integer pair on line 2") !=
                    std::string::npos);
    }

    delete_file(file);
    file = create_file("10 11\n"
                       "12 9\n"
                       "30\n");
    ...
    try {
        ...
        std::string msg = e.what();
        EXPECT_TRUE(msg.find("Invalid integer pair on line 3") !=
                    std::string::npos);
    }
    ...
    try {
        ...
        std::string msg = e.what();
        EXPECT_TRUE(msg.find("Invalid integer pair on line 1") !=
                    std::string::npos);
    }
...
TEST(read_int_pairs, ThrowIfNotEnoughLines) {
    ...
    try {
        ...
        std::string msg = e.what();
        EXPECT_TRUE(msg.find("Expected 3 lines but got 1") !=
                    std::string::npos);
    }
